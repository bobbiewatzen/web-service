...
import com.jsun.site.exception.ResourceNotFoundException;
...
public class AccountRestEndpoint {
	...
	public AccountList read() {
		...
		list.setValue(this.accountService.getAllAcounts());
		return list;
	}
	...
	public ResponseEntity<Account> create(@RequestBody AccountForm form) {
		...
		String uri = ServletUriComponentsBuilder.fromCurrentServletMapping().path("/account/{id}").buildAndExpand(account.getId()).toString();
		HttpHeaders headers = new HttpHeaders();
